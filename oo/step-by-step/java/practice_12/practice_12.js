...
class Class{
    ...
    assignLeader(student){
        if(this.member.indexOf(student.name)==-1){
...
class Teacher extends Person{
    ...
    printMyClassInfo(){
        for(var i=0;i<this.klasses.length;i++){
            for(var j=0;j<this.klasses[i].member.length;j++){
                console.log('I am '+this.name+'. I know ' +this.klasses[i].member[j]
                    +' has joined Class '+this.klasses[i].number);
            }
            console.log('I am '+this.name+'. I know '+this.klasses[i].leader
                +' has become Leader of Class '+this.klasses[i].number);
        }
    }
}
